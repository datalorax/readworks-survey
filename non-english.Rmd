...
d <- d %>% 
  mutate(yrs_teach = case_when(yrs_teach == "43529" ~ "3-5",
                               ...
                               TRUE ~ yrs_teach))
lang_selection <- d %>% 
  select(rspdnt_id, lang_non_eng:other_lang) %>% 
  gather(var, language, - rspdnt_id) %>% 
  drop_na(language)  %>% 
  filter(language != "I don’t know")

d <- semi_join(d, lang_selection)
```
...
ggplot(learn_methods, aes(response, n)) +
  geom_col(fill = "#A39193", alpha = 0.9) +
  coord_flip() +
...
ggplot(d, aes(rate1_10)) +
  geom_bar(fill = "#A39193", alpha = 0.9) +
  coord_flip() +
  labs(y = "Number of Responses",
       x = "")
```
